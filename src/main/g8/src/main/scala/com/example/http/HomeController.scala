...
class HomeController @Inject()(
  ...
  get("/") { _: Request =>
    DemoView(
      "YaFengLi",
      List(Person("test_1", 1), Person("test_2", 2), Person("test_3", 3)))
  }
  ...
  post("/upload") { request: Request =>
    RequestUtils
      ...
      .filter(_._1 == "file")
      .headOption
      .fold(response.notFound) { t =>
        val fn = t._2.filename.get
        val n_fn = RequestUtils
          .multiParams(request)
          .filter(_._1 == "name")
          .headOption
          .fold(fn)(j => new String(j._2.data))
